...
doSub state =
    let ( state', second, first  ) = fsPop2 state 
    in fsPush ( first - second ) state'
...
doSwap state =
    ...
    in fsPush second ( fsPush first state'' )
...
doDrop state =
    ...
    if null ( stack state )
        then state -- stack is empty, return empty stack
        else let ( state', _ ) = fsPop state -- pop head and return tail
            in state'
...
doRot state =
    case stack state of
        ( a:b:c:rest ) -> ForthState { stack = b : c : a : rest }
        _              -> state
...
doDup state = 
    case stack state of
        ( a:rest ) -> ForthState { stack = a : a : rest }
        _          -> state
...
doNode ( While loopBody ) state =
    let ( _, num ) = fsPop state
    in if num /= 0
        then doNode ( While loopBody ) ( doNode loopBody state )
        else state
...
parseExpression' alreadyParsed ( token:tokens ) terminators 
    ...
    | token == Word "if" =
        ...
        in parseExpression' ( alreadyParsed ++ [If { ifTrue = ifTrue, ifFalse = ifFalse }] ) remainingTokens terminators
    ...
    | otherwise =
        let terminal = Terminal token
            ( restParsed, restTokens, lastTerminator ) =
                parseExpression' ( alreadyParsed ++ [terminal] ) tokens terminators
        in ( restParsed, restTokens, lastTerminator )


-- takes the result of parseExpression' and wraps it in an Expression constructor
parseExpression :: [Token] -> AstNode
parseExpression tokens =
    let ( parsedNodes, _, _ ) = parseExpression' [] tokens []
...
parseIf tokens = 
    let ( ifTrue, remainingTokens, terminator ) = parseExpression' [] tokens [ "else", ";" ]
        ( ifFalse, remainingTokens' ) =
            case terminator of
                Just ( Word "else" ) -> parseElse remainingTokens
                _ -> ( Expression [], remainingTokens )
...
parseElse tokens =
    ...
    in ( head ifFalse, remainingTokens )
...
parseWhile tokens =
    let ( body, remainingTokens, _ ) = parseExpression' [] tokens [ ";" ]
    in ( Expression body, remainingTokens )
...
fsPop3 state =
    let ( state', first ) = fsPop state
        ( state'', second ) = fsPop state'
        ( state''', third ) = fsPop state''
    in ( state''', first, second, third ) 

-- return the value on top of the stack 
fsTop :: ForthState -> Float 
fsTop state = last $ stack state 
...
removeComments False nonComments [] = nonComments
...
main = do
    ...
    -- parse the ast 
    let ast = parseExpression $ reverse tokens
